;(function () {
  // Безопасный toString — избегает localeCompare на не-строках
  function str(v) { return v == null ? '' : String(v) }

  function safeLocale(a, b) {
    return str(a).localeCompare(str(b), 'ru')
  }

  // ─── СОСТОЯНИЕ КАТАЛОГА: открытые категории + скролл ─────────────
  const STATE_KEY    = 'novoled_catalog_state'
  const STATE_TTL_MS = 60 * 60 * 1000 // 1 час — потом сброс

  function saveCatalogState() {
    try {
      // Собираем open-состояние всех <details>
      var openPaths = []
      document.querySelectorAll('#catalog-tree details[open]').forEach(function(d) {
        var label = d.querySelector(':scope > summary .tree-node-label span')
  ...
  function restoreCatalogState(state) {
    if (!state) return
    // Открываем категории которые были открыты
    if (state.open && state.open.length) {
      document.querySelectorAll('#catalog-tree details').forEach(function(d) {
        var label = d.querySelector(':scope > summary .tree-node-label span')
        if (label && state.open.indexOf(label.textContent.trim()) !== -1) {
          d.open = true
        }
      })
    }
  ...
  // Сохраняем состояние при уходе со страницы или клике в корзину
  function attachStateSaving() {
    // При любом переходе по ссылке
    document.addEventListener('click', function(e) {
      var link = e.target.closest('a[href]')
      if (link && !link.href.includes('login')) {
        saveCatalogState()
      }
    })
  ...
  function normalizeProduct(row) {
    ...
    return {
      ...
      socket:      str(row.socket),
      power:       str(row.power),
      lumens:      str(row.lumens),
      unit:        str(row.unit),
    ...
    card.innerHTML =
      ...
      '<div class="product-body">' +
        ...
        '<div class="product-price">' +
          '<span class="price-value" style="display:' + (canSee ? '' : 'none') + '">' + priceStr + '</span>' +
          '<span class="price-locked" style="display:' + (canSee ? 'none' : '') + ';font-size:13px;color:var(--text-muted)">Цена после входа</span>' +
        '</div>' +
        ...
        '</div>' +
      '</div>' +
      '<div class="product-bottom"><div class="product-actions">' +
        '<button class="btn-add-to-cart" data-add-to-cart="' + product.id + '" ' +
          (product.in_stock ? '' : 'disabled') + ' type="button">' +
  ...
  function applyFilters(products, searchTerm, seriesSet, socketSet) {
    const term = str(searchTerm).trim().toLowerCase()
  ...
  function renderCategoryTree(tree, container) {
    container.innerHTML = ''
    const isFiltered =
      (str(document.getElementById('search-input') && document.getElementById('search-input').value)).trim().length > 0 ||
    ...
    categories.forEach(function(cat) {
      ...
      details.innerHTML =
        '<summary><div class="tree-node-label"><span>' + cat + '</span></div><span class="tree-node-toggle"></span></summary>'
      const childrenContainer = document.createElement('div')
      childrenContainer.className = 'tree-node-children'
      details.appendChild(childrenContainer)

      // Подкатегории
      Array.from(node.subcategories.keys()).sort(safeLocale).forEach(function(sub) {
        ...
        subDetails.innerHTML =
          '<summary><div class="tree-node-label"><span>' + sub + '</span></div><span class="tree-node-toggle"></span></summary>'
        const subChildren = document.createElement('div')
        subChildren.className = 'tree-node-children'
        subDetails.appendChild(subChildren)

        Array.from(subNode.series.keys()).sort(safeLocale).forEach(function(ser) {
          ...
          serDetails.innerHTML =
            '<summary><div class="tree-node-label"><span>' + ser + '</span></div><span class="tree-node-toggle"></span></summary>'
      ...
      Array.from(node.series.keys()).sort(safeLocale).forEach(function(ser) {
        const serDetails = document.createElement('details')
  ...
  async function initCatalogPage() {
    // Показываем скелетон при каждом вызове (в т.ч. при ручном обновлении)
    if (window.Novoled && window.Novoled.loaders) window.Novoled.loaders.showCatalog()
    ...
    try {
      const raw = await window.Novoled.api.getAllProducts()
    ...
    function render() {
      const filtered = applyFilters(products, searchInput ? searchInput.value : '', selectedSeries, selectedSockets)
    ...
    // Восстанавливаем позицию из предыдущего сеанса
    var savedState = loadCatalogState()
    if (savedState) {
      restoreCatalogState(savedState)
    }

    // Начинаем слушать уходы со страницы
    attachStateSaving()
  ...
  function openProductModal(product) {
      ...
      '<div class="modal-product-layout">' +
        ...
        '<div class="modal-product-info">' +
          ...
          '<div class="product-tags">' +
            (product.series ? '<span class="badge">' + product.series + '</span>' : '') +
            (product.socket ? '<span class="badge">' + product.socket + '</span>' : '') +
            '<span class="badge"><span class="stock-indicator ' + (product.in_stock ? 'stock-available' : 'stock-pending') + '"></span>' + stockLabel + '</span>' +
          '</div>' +
          '<div class="product-specs"><div class="specs-grid">' +
          ...
          '<div class="modal-price-block">' +
            '<div class="product-price-lg">' +
              '<span class="price-value" style="display:' + (canSee ? '' : 'none') + '" id="modal-price-display">' + priceStr + '</span>' +
              '<span class="price-locked" style="display:' + (canSee ? 'none' : '') + ';color:var(--text-muted);font-size:14px">Цена после входа</span>' +
            '</div>' +
            (product.unit ? '<div class="product-unit">Ед.: ' + product.unit + '</div>' : '') +
            (canSee && product.price ? '<div class="modal-total-row"><span class="modal-total-label">Сумма:</span><span class="modal-total-value" id="modal-total">' + priceStr + '</span></div>' : '') +
          '</div>' +
          '<div class="product-detail-actions">' +
            '<div class="modal-qty-row">' +
              '<div class="qty-stepper">' +
                '<button type="button" class="qty-btn" id="modal-qty-minus" aria-label="Минус"><span class="qty-icon">&#8722;</span></button>' +
                '<div class="qty-value" id="modal-qty-value">1</div>' +
                '<button type="button" class="qty-btn" id="modal-qty-plus" aria-label="Плюс"><span class="qty-icon">+</span></button>' +
              '</div>' +
              '<button class="btn-primary' + (product.in_stock ? '' : ' btn-disabled') + '" id="modal-add-to-cart" type="button" ' + (product.in_stock ? '' : 'disabled') + '>' +
                (product.in_stock ? 'В корзину' : 'Ожидается') +
              '</button>' +
            '</div>' +
    ...
    if (addBtn && product.in_stock) {
      addBtn.addEventListener('click', function() {
        if (window.Novoled && window.Novoled.addToCart) window.Novoled.addToCart(product, modalQty)
        addBtn.classList.remove('btn-pulse')
        void addBtn.offsetWidth
        addBtn.classList.add('btn-pulse')
