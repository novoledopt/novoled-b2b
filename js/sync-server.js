/**
 * sync-server.js — СЕРВЕРНЫЙ скрипт синхронизации Google Таблицы → Supabase
 *
 * Запускается через GitHub Actions (автоматически 2 раза в день).
 * SERVICE_KEY берётся из переменных окружения — НЕ хранится в коде!
 *
...
if (!SUPABASE_URL || !SERVICE_KEY || !SHEET_URL) {
  console.error('❌ Не заданы переменные окружения SUPABASE_URL / SUPABASE_SERVICE_KEY / SHEET_URL')
  process.exit(1)
}
